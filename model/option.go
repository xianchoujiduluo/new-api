...
func InitOptionMap() {
	...
	common.OptionMap["RecordPayloadEnabled"] = strconv.FormatBool(common.RecordPayloadEnabled)
...
func validateOptionValue(key string, value string) error {
	...
	if key == frontend_setting.DownloadProxyOptionKey {
		return frontend_setting.ValidateDownloadProxy(value)
	}
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "RecordPayloadEnabled":
			common.RecordPayloadEnabled = boolValue
