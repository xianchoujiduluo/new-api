...
type ChannelOtherSettings struct {
	...
	// ConvertResponsesToChat accepts OpenAI Responses requests from clients that
	// only speak /v1/responses (Codex, for example) and translates them into Chat
	// Completions for an upstream that only implements /v1/chat/completions.
	// Off by default: requests are forwarded natively without conversion.
	ConvertResponsesToChat                bool                  `json:"convert_responses_to_chat,omitempty"`
